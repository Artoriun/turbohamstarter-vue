...
/**
 * Thrown when the sign-in is throttled. `retryAfter` is the seconds left, present only for
 * the fixed lockout: the two window-based limits behind it cannot say when they will let go.
 *
 * Separate from the deliberate "one message for everything else" rule in the form: that rule
 * exists so an attacker cannot tell a wrong password from an unreachable API, and it still
 * holds. A lockout is different — the person being told about it is the one who caused it,
 * so the countdown reveals nothing they did not already know, and withholding it makes a
