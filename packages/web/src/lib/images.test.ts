...
const CLOUDINARY = 'https://res.cloudinary.com/dgk299isx/image/upload/v1234/poem_abc.png';

describe('optimizeUrl', () => {
  test('injects format, quality and width', () => {
    assert.equal(
      optimizeUrl(CLOUDINARY, 400),
      'https://res.cloudinary.com/dgk299isx/image/upload/f_auto,q_auto,w_400/v1234/poem_abc.png',
    );
  });
  ...
  test('takes a raw URL: applying it twice chains transforms rather than replacing', () => {
    // Not idempotent, and deliberately not made so — every caller passes a raw poem.image
