...
test('a root link followed from a Japanese page stays in Japanese', async ({ page }) => {
  // The language prefix lives in the router's basename, not in the route table, so `to="/"`
  // is supposed to resolve to /ja/ on its own. That is the whole argument for the basename
  // scheme, and nothing was checking it — every root link on the site (the brand, the
  // not-found button, the error fallback) depends on it silently.
  ...
  // The not-found page's way back is the same kind of link, reached the same way.
